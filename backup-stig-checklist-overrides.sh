...
function override_stig_check() {
  ...
  original_stig_data=$(grep -Pzo -m 1 "(?s)<VULN>.*?${$id}.*?</VULN>" "$stig_path" | tr -d '\000')
  ...
  formatted_original_stig_data=$(echo "$original_stig_data" | sed 's@\*@\\*@g' | sed 's@\"@\\"@g' | sed -z 's@\n@\\n@g' | sed 's@</VULN>\\n@</VULN>@g')
  ...
  # grep -Poz "$formatted_original_stig_data" "$stig_path"
  sed -iz "s@${formatted_original_stig_data}@${new_stig_data}@g" "$stig_path"
}
